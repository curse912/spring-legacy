		...
		<div class="innerOuter">
			...
			<!-- 
				...
				1. application/x-www-form-urlencoded(기본값)
				 ...
				 - 단, 전달할 데이터중 file형태으이 데이터가 포함된 경우 전송이 불가능
				2. multipart/form-data
				 - 파일 업로드시 반드시 사용해야하는 enctype
				 - 텍스트 데이터와 바이너리형태의 데이터(파일)를 개별적인 파트로 나누어 각각 따로 전송하는 방식
				 - 서버는 멀티파트 데이터로 요청한 데이터에 대해서는 별도의 파싱과정을 거쳐야만 브라우저에서 던달한 파람값들에 대해 접근이 가능
				 - Spring 기반 서버에서는 MultipartResolver를 등록하여 HttpServletRequest객체를 MultipartHttpServletRequest로 변환처리해야 파람에 접근 가능하다.
			 -->
