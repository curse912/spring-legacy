...
public class BoardDaoImpl implements BoardDao{
	...
	public Map<String, String> getBoardTypeMap() {
		 ...
		 *  - 두번째 매개변수로는 어떤 '칼럼'을 key로 사용하지를 작성
	...
	public List<Board> selectList(PageInfo pi, Map<String, Object> paramMap) {
		 ...
		 *  SELECT*
		 *  FROM ( SELECT ROWNUM AS RNUM, INNER.*
		 * 		   FROM (
		 * 				 -- 특정 칼럼을 기준으로 조회된 쿼리
		 * 	 		   )INNER
		 * 	 	 )
		 *  WHERE RNUM BETWEEN A AND B
		 *  - 쿼리분이 복잡하고, 코드의 가독성이 떨어지나 필요한 행만 조회하여 가져올 수 있기 때문에
		 ...
		 *  - 전체 쿼리결과를 자바 어플리케이션으로 가져온 후, 지정한 위치(offset)에서 
		 *    특정 개수(limit)를 잘라내는 방식으로 페이징 처리를 진행한다.
		 *  - 어플리케이션으로 가져올 데이터가 수만건 이상인 경우 메모리 낭비 및 성능저하가 발생 할 수 잇다.
		 ...
		 * 		...조건절
		 * 	 ORDER BY 절
		 * 	 OFFSET 시작행 ROWS FETCH ENXT 조회할개수 ROWS ONLY  
		 * */
		// 몇 번째 행부터 가져올지
		int offset = (pi.getCurrentPage() - 1) * pi.getBoardLimit();
		int limit = pi.getBoardLimit(); //offset위치에서 부터 몇개의 행을 가져올지
