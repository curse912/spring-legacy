...
public class MemberController {
	...
	/*
	 * Spring의 DI(Dependency Injection) - 의존성 주입 - 어플리케이션을 구성하는 객체를 개발자가 직접 생성하는게
	 * 아닌, 스프링이 생성한 객체를 주입받아서 사용하는 방식 - new 연산자를 직접 사용하지 않고, 자료형 선언만 한
	 * 후 @Autowired어노테이션을 통해 주입받음
	 */
...
//	}
	...
	// @RequestParam 어노테이션
	// server의 request.getParameter("키")로 뽑은 역할을 대신 수행해주는 어노테이션
	// 클라이언트가 요청한 값을 대신 반환하여 바인딩 해주는 역할은 argumentResolver가 수행.
//		@RequestMapping(value="/member/login", method = RequestMethod.GET)
...
//	}
	...
	// @RequestParam생략
	// 매개변수의 이름과 일치하는 request의 파라미터값을 추출하여 바인딩
...
//	}
	...
	/*
	 * 커맨드 객체 방식 - 메소드의 매개변수로 vo클래스 타입을 지정하는 경우, 요청 전달값의name속성과 일치하는 VO클래스 필드의
	 * setter함수를 호출하여 바인딩
	 * 
	 * 매개변수의 클래스와 일치하는 클래스의 기본생성자 호출
	 * 파라미터의 key값과 클래스의 필드명이 일치하는 경우setter함수 호출;
	 */
//	@RequestMapping(value = "/member/login", method = RequestMethod.POST)
...
//		 * */
//		model.addAttribute("errorMesg","오류발생");
//		
//		mv.addObject("errorMsg","오류발생");
//		mv.setViewName("coommon/errorPage");
//		
//		return mv;
//	}
	...
	public ModelAndView login(@ModelAttribute Member m, ModelAndView mv, Model model, HttpSession session	/*로그인성공시, 사용자 정보를 보관할 객체*/, RedirectAttributes ra ) {
		//로그인 요청 처리
		...
		if(loginUser != null) {
			 ...
			 * 리다이렉트가 완료된면, sessionScope에 있던 데이터를 requestScope로 변경
			 * */
		}else {
	...
	public String insertMember(Member m, Model model, RedirectAttributes ra) {
		...
		if(result > 0) {
			ra.addFlashAttribute("alertMsg","회원각입 성공^^");
			viewName = "redirect:/";
		}else {
			model.addAttribute("errorMsg","회원가입 실패ㅠㅜㅠ");
	...
	public String updateMember(Member m, RedirectAttributes ra, Model model ) {
...
//		throw new RuntimeException("예외발생");
		...
		if(result > 0) {
			...
			//업데이트 실패시
			ra.addFlashAttribute("errorMsg","회원정보 수정 실패");
	 ...
	 *  2. 하나의 커늩롤러에서 발생하는 예외들을 모아서 처리하는 방법 --> 2순위로 적용
	 *  	컨트롤러에 예외처리 메서드를 1개 추가한후, @ExceptionHandler 추가
	 *  3. 어플리케이션 전역에서 발생하는 예외를 모아서 처리하는 방법 --> 3순위로 적용
...
//	}
	...
	// 비동기 요청
	@ResponseBody	//반환되는 값이 값 그자체임을 의미하는 주석(어노테이션)
...
//	//jackson-databind를 활용하여, vo클래스, AraayList, map데이터를 자동으로 JSON형태로 바인딩하기.
//	return map;
//	}
...
//		ResponseEntity res = null;
		...
		if(map != null) {
			res = ResponseEntity.ok()	// 응답상태 200
								.header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_UTF8_VALUE)
								.body(map);
		}else {
			res = ResponseEntity.notFound()	//응답상태 404
								.build();
		}
		return res;
	}
